...
def update_contextual_attention_model(mode):  # 参数分别代表了不同的模型
    ...
    # reshape
    merged_attention1 = Lambda(lambda x: K.reshape(x, (-1, max_utt_len, 3 * dense_units)))(merged_attention1)  # TODO：这里改了
    ...
    # reshape
    merged_attention_two_combine = Lambda(lambda x: K.reshape(x, (-1, max_utt_len, 2 * dense_units)))(merged_attention_two_combine)
